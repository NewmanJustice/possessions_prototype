# Test Behaviour Matrix — Screen 28: Claiming Costs
...
**Behaviour:**
- T-1.1: Page displays heading "Claiming costs"
- T-1.2: Caption displays "Make a claim"
- T-1.3: Current case number displayed above heading
- T-1.4: Page is accessible at `/claims/claiming-costs`
...
**Behaviour:**
- T-2.1: Question "Do you want to ask for your costs back?" displayed
- T-2.2: Hint text displays: "You do not need to provide the exact amount at this stage, but a judge will request a schedule of costs at the hearing"
- T-2.3: Radio option "Yes" displayed
- T-2.4: Radio option "No" displayed
- T-2.5: Radio options use correct name attribute (`claimingCosts`)
- T-2.6: Only two options available (no "Other" or "Unsure" option)
...
- T-3.2: Error message: "Select yes if you want to ask for your costs back"
...
### AC-4: Persist costs claim intention

**Behaviour:**
- T-4.1: Yes selection stores `claimingCosts: 'yes'`
- T-4.2: No selection stores `claimingCosts: 'no'`
- T-4.3: Value stored in `session.claim.claimingCosts`
...
**Behaviour:**
- T-5.1: Yes pre-selected when `claimingCosts === 'yes'`
- T-5.2: No pre-selected when `claimingCosts === 'no'`
...
**Behaviour:**
- T-6.1: Clicking Previous redirects to `/claims/statement-of-express-terms` (Screen 26d)
...
**Behaviour:**
- T-7.1: Continue with Yes selection redirects to next screen (TBD)
- T-7.2: Continue with No selection redirects to next screen (TBD)
...
- T-9.2: Error link targets radio group (`#claimingCosts`)
...
### Entry Condition
- T-ENTRY-1: Page accessible when arriving from `/claims/statement-of-express-terms`
...
**Q1 - Next screen route:** Continue navigation currently routes to TBD; confirm next screen before go-live
**Q2 - Form field name:** Confirm field name is `claimingCosts`
...
|---------------------|----------|-------|
| AC-1 | T-1.1, T-1.2, T-1.3, T-1.4 | Page heading, caption, case number, URL |
| AC-2 | T-2.1, T-2.2, T-2.3, T-2.4, T-2.5, T-2.6 | Question, hint text, radio options |
